...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- luxcoder: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via luxcoder bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'iboss21/LUXCode';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'luxcoder-bug-reporter',
    });
