/* ============================================================
   ...
   Cursor + v0.app aesthetic on top of LXR NUI system
   Drop lxr-settings.css + this file, wire from your Lua
   ============================================================ */
...
(function () {
  ...
  // Declarative schema — "everything" in one place.
  // Other LXR resources can push additional categories at runtime via LXRSettings.registerCategory(cat)
  const DEFAULT_SCHEMA = [
    ...
    },
    {
      ...
      icon: '⚡',
      desc: 'One-click tuning for busy vs immersive servers. Changes multiple medical values.',
      fields: [
    ...
    },
    {
      ...
      fields: [
        ...
        { key: 'staggerNui', type: 'number', label: 'NUI Stagger (ms)', desc: 'Delay between heavy SendNUIMessage chunks.', default: 70, min: 0, max: 200 },
      ]
    },
    {
      id: 'about',
  ...
  // Public API — other resources or your own code can extend
  window.LXRSettings = {
    ...
    open(initialConfig, localeBundle) {
      ...
      state.open = true;
      renderAll();
      // focus search for power users (Cursor vibe)
  ...
  function ensureDOM() {
    ...
    container.innerHTML = `
      <div class="lxr-medic-layout">
        <div class="lxr-settings-top" style="grid-column: 1 / -1; padding: 0 0 8px 0; display:flex; align-items:center; gap:12px;">
          ...
          <input class="lxr-settings-search" placeholder="Search settings... (type / to focus)" />
    ...
    // Insert into the main adm-main area (same as boss / admin tabs)
    ...
    // Keyboard: / focuses search (Cursor muscle memory)
  ...
  function renderContent() {
    ...
    state.schema.forEach(cat => {
      // Filter category if search active
      ...
      // If a specific category is chosen and no search, only show that one
      ...
      visibleFields.forEach(field => {
        ...
        if (field.type === 'toggle') {
          ...
          const opts = field.options.map(o => `<option value="${o.v}" ${o.v==currentVal?'selected':''}>${o.l}</option>`).join('');
          ...
          controlHTML = `
            <div style="max-width:420px; font-size:12px; color:var(--muted);">
              ...
              Cursor-grade settings for RedM. All changes are sent live to the server.
            </div>
          `;
        }
        ...
        // Profile buttons (smart bulk apply)
    ...
    if (q) {
      // highlight matches
      const marks = root.querySelectorAll('.lxr-setting-title, .lxr-setting-desc');
      // simple client side highlight is already handled by the filter; we can enhance later
    }
  }
  ...
  function applyProfile(name) {
    ...
    state.dirty = true;
    updateDirty();
    renderContent(); // re-render numbers
  }
  ...
  function applyAndSave() {
    ...
    // small toast feel
    const orig = container.querySelector('#lxrSettingsApply').textContent;
    const b = container.querySelector('#lxrSettingsApply');
    b.textContent = 'Saved ✓';
    setTimeout(() => { if (b) b.textContent = orig; }, 1200);
  }
    ...
    inp.onchange = () => {
      ...
      r.onload = () => {
        try {
          ...
          state.dirty = true;
          renderContent();
          postNUI('settings:updateMany', { updates: data });
  ...
  // Boot: listen for messages from Lua (consistent with your existing app.js pattern)
  window.addEventListener('message', (e) => {
    const d = e.data || {};
    if (d.action === 'openSettings' || d.action === 'settings:open') {
      const cfg = d.config || d.settings || {};
      const loc = d.locale || {};
      window.LXRSettings.open(cfg, loc);
    }
    if (d.action === 'settings:apply') {
      // server pushed new values (e.g. after another admin changed them)
      if (d.config) {
  ...
  // Optional: expose a helper so your existing admin code can open it
  // window.LXRMedicAdmin && window.LXRMedicAdmin.openSettings = () => window.LXRSettings.open(...)

  console.log('%c[LXR] Cursor-grade Settings module loaded', 'color:#c9a96e');
})();
